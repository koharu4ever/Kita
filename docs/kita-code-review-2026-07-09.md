...
| 环境配置            | `src/config`                                  | 集中管理方向正确；`SKIP_ENV_VALIDATION` 的布尔解析仍是待修 P1                   |
...
下一步更值得优先处理的是 `SKIP_ENV_VALIDATION` 布尔解析、production Compose 默认凭据，以及内容和产品层收口。backup last-success、恢复演练、临时数据库和 Playwright 属于有价值但可分阶段实施的增强项。
