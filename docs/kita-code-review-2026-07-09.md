...
| 环境配置            | `src/config`                                  | 集中管理方向正确；`SKIP_ENV_VALIDATION` 已在本 PR 改为严格布尔解析并增加测试    |
| 基础设施            | Compose、`docker`                             | Web、PostgreSQL、备份职责分离，生产备份已有运行与自动测试证据                   |
| 测试和合并门禁      | Vitest、backup shell、GitHub Actions、Ruleset | 30 个 Vitest 用例和 4 个 shell 用例已进入必需的 `quality` 检查                  |
...
| 优先级  | 问题                                                                | 影响                                                                                                                                      | 当前判断                                                        |
| ------- | ------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| P1      | `src/config/env.ts` 用 `Boolean(process.env.SKIP_ENV_VALIDATION)`   | 字符串 `"false"` 也会被当成 `true`，可能意外跳过环境校验；而 `payload.config.ts` 已使用严格比较，两处行为不一致                           | **修复已实现，待本 PR 合并**                                    |
...
临时 PostgreSQL、Playwright、恢复演练和 backup last-success 都有价值，但以当前内容量不应排在上述三项之前。本 PR 合并后，最合理的方向是：**单独处理 production Compose fail-fast，然后停止工程底座扩张，开始完成内容和产品体验。**
...
P1-3 第一阶段关闭后，当前短板主要集中在两个 P1 配置细节、内容、性能、可访问性、错误/空状态和 SEO。备份最近成功时间监控、恢复演练、secret 轮换及更深的集成/E2E 测试保留为后续增强，不再作为当前阻断。
...
| 测试与发布门禁 | 8/10 | 30 个 Vitest 与 4 个 backup shell 用例、GitHub Actions `quality` 和 main Ruleset 已落地；集成/E2E 仍待补。 |
...
### P1-1：`SKIP_ENV_VALIDATION=false` 仍会跳过校验 — **修复完成，待 PR 合并（2026-07-13）**
...
4. `SKIP_ENV_VALIDATION` 严格解析已在本 PR 修复；production fail-fast 仍列在 P1，不影响 P0-1 关闭。
...
P1-1 的 `SKIP_ENV_VALIDATION` 布尔解析已在本 PR 修复，合并后可关闭。下一步更值得优先处理的是 production Compose 默认凭据，然后转向内容和产品层收口。backup last-success、恢复演练、临时数据库和 Playwright 属于有价值但可分阶段实施的增强项。
