...
| ---- | ------------------------------ | --------------------------------------------- | ------ | --------------------------- |
| 1    | 字段验证与 Admin 输入约束      | 阻止坏 slug、坏 URL 和错误发布日期进入数据库  | 低到中 | Media 后优先做              |
| 2    | 显式 create/update/delete 权限 | 让 API 和 Admin 写权限可审计，不依赖默认行为  | 低     | 与字段验证同批或单独小 PR   |
| 3    | PostgreSQL migration smoke     | 证明全部 migration 能从空库建立真实 schema    | 低到中 | 在下一次复杂 migration 前做 |
| 4    | Trash / 软删除                 | Games、Reviews、Media 误删后可从 Admin 恢复   | 低到中 | Media 稳定后做              |
| 5    | 共用 rich-text 配置            | 消除 Games/Reviews Lexical 配置漂移           | 低     | 修改 collection 时顺手完成  |
| 6    | Reviews -> Media relationship  | 统一 review card 和详情图片                   | 中     | Games Media 稳定后做        |
...
实施前的 36 个 Vitest 对 mapper、getter、seed 和环境变量已有良好覆盖，但 getter 测试使用 mocked Payload client。本轮补充 Media 配置、权限和映射测试后，仍应继续建设真实数据库 smoke。现有 GitHub Actions：
...
- 四条历史 migration 能否在全新 PostgreSQL 16 上完整执行；
- 新增 Media、relationship、date 等 migration 是否与真实数据库兼容；
...
```text
P1  Media + R2 + Games.cover 兼容接入
  ↓
真实图片迁移与生产验证
  ↓
P2  CI PostgreSQL migration smoke
  ↓
P3  slug / URL validation + 显式写权限
  ↓
P4  releaseDate date field 数据迁移
  ↓
...
P7  Reviews.cover -> Media
  ↓
P8  Reviews.game -> Games relationship
  ↓
P9  About Global（真实正文）
```

共用 rich-text 配置不需要占用一个独立阶段，在 P3、P7 或其他同时修改 Games/Reviews Collection 的 PR 中顺手完成即可。

这不是要求连续完成九个 PR。推荐实际停点是：

```text
完成 P1
  -> 先正常使用 Media
  -> 有空再完成 P2/P3/P4/P5 的小型质量增强
  -> 只有真实 Reviews/About 内容开始增长时才进入 P7/P8/P9
```

每一步都必须回答一个已经存在的问题。没有出现相应问题时，不继续扩张 Payload 配置。P2 到 P5 可以提升正确性和恢复能力，但都不应阻止继续录入内容。

## 21. 最终建议

批准实施这个 PR，但使用以下定义：

> 为 Kita 新增 Payload Media collection，并通过官方 S3 adapter 把生产公开图片保存到独立 Cloudflare R2 bucket；Games 以 nullable upload relationship 接入 Media，mapper 保留现有静态封面回退，从而在不破坏当前内容和前端 DTO 的前提下验证完整媒体链路。

这个方案确实使用了 Payload 的核心后端能力：
...
- 有明确 fallback、回滚和后续清理边界。

因此，它是目前最适合 Kita 的下一项 Payload 后端增强。
