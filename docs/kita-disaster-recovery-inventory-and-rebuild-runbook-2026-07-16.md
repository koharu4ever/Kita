...
| Volume name           | `<actual Coolify volume name>`    |
...
- [ ] 备份 D 盘重要文件；
- [ ] 保存当前 Kita 未提交文件；
- [ ] 确保 GitHub `main` 有最新已合并代码；
- [ ] 确认主邮箱可恢复；
- [ ] 确认 GitHub、Cloudflare、VPS provider 可以登录；
- [ ] 建立密码管理器目录；
- [ ] 记录域名注册商和到期日。
...
- [ ] 录入 Kita 15 个配置键；
- [ ] 录入真实 secret 到密码管理器；
- [ ] 录入 OpenList admin 和 storage credentials；
- [ ] 录入 Coolify APP_KEY 的安全位置；
- [ ] 备份 Coolify SSH keys；
...
- [ ] 创建私有 `kita-recovery-private` bucket；
- [ ] 使用独立 token；
...
- [ ] 保存 APP_KEY 和 SSH keys；
- [ ] 验证 PostgreSQL backup 最新对象；
- [ ] 制作 OpenList data backup；
- [ ] 记录 DNS；
