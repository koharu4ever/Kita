...
### 1.1 2026-07-21 当前完成度快照

这份快照用于区分“材料已收集”“单项已验证”和“完整恢复已演练”。表内提交号是 2026-07-20 演练时的固定证据，不代表当前 `main` HEAD；当前提交应以 Git 为准。任何后续文档都应引用本节，不应把未完成项改写成已闭环。

| 项目                                    | 状态           | 证据/边界                                                                             |
| --------------------------------------- | -------------- | ------------------------------------------------------------------------------------- |
| GitHub `main` 与本地 `main`（演练基线） | 已核对         | 2026-07-20 演练时均为 `78ad2d2`；当时工作区干净，无本地唯一项目文件                   |
...
| Payload Media / R2                      | 本地已验证     | migration、上传/尺寸/清理、standalone 启动已通过；独立生产 bucket 和上传 smoke 待完成 |
...
| 键名                         | 是否 secret      | 恢复说明                                    |
| ---------------------------- | ---------------- | ------------------------------------------- |
| `MEDIA_STORAGE_MODE`         | 否               | 本地为 `local`；production 必须为 `r2`      |
| `MEDIA_R2_BUCKET`            | 否               | 必须与 PostgreSQL backup bucket 分离        |
| `MEDIA_R2_ENDPOINT`          | 敏感配置但非密码 | R2 S3-compatible endpoint                   |
| `MEDIA_R2_PUBLIC_URL`        | 否               | 独立 custom domain；Docker build 和 runtime |
| `MEDIA_R2_ACCESS_KEY_ID`     | 凭据             | 仅保存于密码管理器/Coolify runtime          |
| `MEDIA_R2_SECRET_ACCESS_KEY` | 是               | 仅保存于密码管理器/Coolify runtime          |
...
- [ ] Media bucket 与 PostgreSQL backup bucket 不同；
- [ ] Media token 只读写 Media bucket，不复用 backup token；
- [ ] `MEDIA_R2_PUBLIC_URL` 指向已启用的 HTTPS custom domain；
- [ ] production `MEDIA_STORAGE_MODE=r2`，且没有回退到容器本地目录；
...
### 6.5 Payload Media 数据资产

Media 接入后，PostgreSQL 只保存 Media 元数据和 Games relationship，图片二进制保存在独立 R2 media bucket。PostgreSQL dump 不包含这些图片对象。

恢复 inventory 必须记录：

| 字段                 | 说明                                             |
| -------------------- | ------------------------------------------------ |
| Media bucket         | 独立 bucket 名；不能复用 `kita-postgres-backups` |
| Public custom domain | `MEDIA_R2_PUBLIC_URL` 对应域名                   |
| S3 endpoint          | 对应 Cloudflare account                          |
| Token scope/location | bucket-scoped read/write；真实值只在安全位置     |
| Last upload smoke    | 日期、对象 key 和结果；不记录 secret             |
| Independent backup   | 当前尚未演练，不得写成已闭环                     |

推荐恢复顺序：先恢复应用和 PostgreSQL，再重新连接或恢复 media bucket，随后验证 Media URL、Games 列表/详情和重新部署后的图片持久性。删除 Media document 前应先解除内容引用；第一阶段不做自动对象 GC。
