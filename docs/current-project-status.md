...
- Payload Media + Games.cover 兼容链路；本地文件与 Cloudflare R2 使用同一 Media schema。
...
ESLint                通过
TypeScript            通过
Vitest                11 files / 46 tests 通过
Backup shell tests     4 scenarios 通过
当前 main 基线        17d0995（PR #16 已合并）
...
核心功能与生产链路已经建立
  ...
  -> 46 个 Vitest + 4 个 backup shell 场景进入 CI
  -> main 由 Pull Request + quality Ruleset 保护
  -> 当前重点转向真实内容、产品体验和少量高价值集成测试
```

Payload Media 当前边界：代码、本地 storage、真实图片尺寸生成、migration 链路和 R2 模式 standalone 启动已验证；standalone 使用的是假 R2 配置，没有上传远端对象。Cloudflare media bucket、custom domain、Coolify 变量、生产上传和真实 Games 封面迁移尚未完成，不能写成 R2 Media 已上线。
...
- Games migrations。
- 封面资源与数据库 schema 解耦。
- 可选 Payload Media `cover` relationship。
- Media 不可用或未绑定时继续读取旧 `coverSrc`/`coverAlt`/宽高字段。
